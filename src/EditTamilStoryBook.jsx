...
export function EditTamilStoryBook({ tamilStory, setTamilStory }) {
  const navigate = useNavigate();
  const { id } = useParams();

  const selectedUser = tamilStory.find((hi) => hi.id === id);
  ...
  const updateBook = (val) => {
    const editedIndex = tamilStory.findIndex((hi) => hi.id === id);
    tamilStory[editedIndex] = val;
    setTamilStory([...tamilStory]);
