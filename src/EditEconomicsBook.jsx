...
export function EditEconomicsBook({ economics, setEconomics }) {
  const navigate = useNavigate();
  const { id } = useParams();

  const selectedUser = economics.find((hi) => hi.id === id);
  ...
  const updateBook = (val) => {
    const editedIndex = economics.findIndex((hi) => hi.id === id);
    economics[editedIndex] = val;
    setEconomics([...economics]);
    navigate("/library/economics");
  };
