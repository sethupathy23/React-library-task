...
export function EditHistoryBook({ history, setHistory }) {
  const navigate = useNavigate();
  const { id } = useParams();

  const selectedUser = history.find((hi) => hi.id === id);
  ...
  const updateBook = (val) => {
    const editedIndex = history.findIndex((hi) => hi.id === id);
    history[editedIndex] = val;
    setHistory([...history]);
